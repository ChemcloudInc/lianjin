...
if not rs.eof then%>
<form name="form1" method="post" action="save_order.asp?action=saveall&ordernum=<%=ordernum%>">
...
<option value="Unsend" <%if trim(rsp("zhuangtai"))="Unsend" then%>selected<%end if%>>Unsend</option>
<option value="Send" <%if trim(rsp("zhuangtai"))="Send" then%>selected<%end if%>>Send</option>
</select></td>
<td><textarea name="beizhu" cols="30" rows="3" id="beizhu"><%=rsp("beizhu")%></textarea></td>
<td><%if rsp("zhuangtai")="Unsend" then%><a href="save_order.asp?action=delxd&ordernum=<%=ordernum%>" class="linkurl">Delete</a><%else%>No Operate<%end if%></td>
</tr>
...
<a href="view_order.asp?action=del&ordernum=<%=ordernum%>&orderid=<%=rs("orderid")%>" onClick="return confirm('Are you sure delete？')">Delete</a>
